...
from ..utils import insert_missing_annotations, compute_and_save_text_embeddings


class TREC6Sim(MultiAnnotatorDataset):
    def __init__(
        self,
        root: str,
        download: bool = False,
        ...
        # Download data.
        data_dir = os.path.join(root, "trec_6_bert_embeddings")
        ...
        if version in ["train", "valid"]:
            train, valid = train_test_split(
                np.arange(len(self.y)), test_size=500, random_state=0, stratify=self.y
            )
            if version == "train":
                self.x, self.y = self.x[train], self.y[train]
                annotation_indices = torch.arange(0, 4952)
            else:
                self.x, self.y = self.x[valid], self.y[valid]
                annotation_indices = torch.arange(4952, 5452)
        else:
            annotation_indices = torch.arange(5452, 5952)
